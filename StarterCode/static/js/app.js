...
// ----------------------------------------------------------------------------
  function barChart(subjectId){
      ...
      d3.json('samples.json').then((data)=>{
          ...
          var ID = dataSamples.map(row=>row.id).indexOf(subjectId);
          var otuValueTopTen = dataSamples.map(row=>row.sample_values);
          var otuValueTopTen = otuValueTopTen[ID].slice(0,10).reverse();
          var otuIdTopTen = dataSamples.map(row=>row.otu_ids);
          var otuIdTopTen = otuIdTopTen[ID].slice(0,10);
          var otuLabelTen = dataSamples.map(row=>row.otu_labels).slice(0,10);
